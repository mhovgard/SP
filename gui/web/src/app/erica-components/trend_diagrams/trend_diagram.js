...
var TrendDiagram = (function () {
    function TrendDiagram() {
    }
    TrendDiagram.prototype.scaleData = function (data, minMinutes, maxMinutes) {
    ...
    TrendDiagram.prototype.checkYLimit = function (dataPoint) {
    ...
    TrendDiagram.prototype.draw = function (data, selector, ylims) {
        xAxisLimits.min = 0;
        xAxisLimits.max = 0;
        yAxisLimits.min = 0;
        yAxisLimits.max = 0;
        ...
        var margin = { top: 20, right: 40, bottom: 20, left: 40 }, width = 550 - margin.left - margin.right, // width 350 förut
        ...
        var xTimes = ["-120 min", "-90min", "-60min", "-30min", "0min", "+30min"];
        var xstr = d3.scale.ordinal()
            .domain(xTimes)
            .rangePoints([0, width]); //([xAxisLimits.min, xAxisLimits.max]); //([0,width]);
        ...
        var y = d3.scale.linear()
            .range([0, height]);
        //.range([yAxisLimits.min, yAxisLimits.max]); //funkar inte
        ...
        if (ylims[0] < maxValue || ylims[1] > minValue) {
            ...
            else if (ylims[0] < maxValue && ylims[1] < minValue) {
                yellowY = y(ylims[0]);
                yellowHeight = height - y(ylims[0]);
            }
            else if (ylims[1] > minValue) {
    ...
    TrendDiagram.prototype.drawCircles = function (parent, data, x, y) {
        var svg = parent;
        //---------------------------------------------------//
        //-------------------- circles ----------------------//
        var smallR = 5;
...
exports.TrendDiagram = TrendDiagram;
//# sourceMappingURL=trend_diagram.js.map